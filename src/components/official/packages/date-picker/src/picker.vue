<template>
  ...
  <div
    class="k-date-editor k-range-editor k-input__inner"
    :class="[
      'k-date-editor--' + type,
      pickerSize ? `k-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['k-input__icon', 'k-range__icon', triggerClass]"></i>
    <input
      ...
      @focus="handleFocus"
      class="k-range-input">
    <span class="k-range-separator">{{ rangeSeparator }}</span>
    <input
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="k-input__icon k-range__close-icon">
    </i>
  </div>
</template>
...
export default {
  ...
  watch: {
    pickerVisible(val) {
      ...
      if (val) {
        ...
        this.userInput = null
        this.dispatch('KFormItem', 'KForm.blur')
        this.$emit('blur', this)
        this.blur()
      }
    },
  ...
  computed: {
    ...
    triggerClass() {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'k-icon-time' : 'k-icon-date')
    },
  ...
  methods: {
    ...
    emitChange(val) {
      ...
      if (val !== this.valueOnOpen) {
        ...
        this.dispatch('KFormItem', 'KForm.change', val)
